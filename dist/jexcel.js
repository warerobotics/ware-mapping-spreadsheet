...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            // NEW EVENTS to handle on mouse hover row
            onmouseenter:null,
            onmouseleave:null,
        ...
        obj.createRow = function(j, data) {
            ...
            // Add events support to each table row
            obj.rows[j].onmouseenter = onmouseenter;
            obj.rows[j].onmouseleave = onmouseleave;

            function onmouseenter(event) {
                obj.dispatch('onmouseenter', event, el, obj.rows[j], j, obj.options.data);
            }

            function onmouseleave(event) {
                obj.dispatch('onmouseleave', event, el, obj.rows[j], j, obj.options.data);
            }
        ...
        obj.createCell = function(i, j, value) {
            ...
            // NEW Modified by Rodrigo Viera to fix overflow issue on modals components
            // Overflow
            // if (i > 0) {
        ...
        obj.setMerge = function(cellName, colspan, rowspan, ignoreHistoryAndEvents) {
            ...
            if (test) {
                ...
                // NEW modified by rodrigo viera to fix issue
                // Overflow
        ...
        obj.openEditor = function(cell, empty, e) {
            ...
            // NEW Modified by Rodrigo Viera to fix the overflow issue on open modal components
            // Overflow
            // if (x > 0) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'hidden') {
                        ...
                        if (obj.options.columns[x].options && obj.options.columns[x].options.type) {
                            // console.log('obj.options.columns[x].options.type ', obj.options.columns[x].options.type)
                        ...
                        // console.log('obj.options.data[y][x] ', obj.options.data[y][x]);
                        // console.log('obj.options.data[y][x] trimmed ', obj.options.data[y][x].trim());
                        // console.log('4 valueeee type of ', typeof(value));
                        ...
                        editor.onblur = function() {
                            // console.log('onblur aca 4 ::::: ',value)
                            obj.closeEditor(cell, true);
                        };
                        editor.focus();
                        // console.log('aca 4 ::::: ',value)
                        editor.value = value;
                    }
                }
            }
        ...
        obj.closeEditor = function(cell, save) {
            ...
            if (save == true) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio' || obj.options.columns[x].type == 'hidden') {
                        ...
                        // Added by rodrigo the spaces removement
                        var value = cell.children[0].value.trim().replace(/ +/g, '');;
                        cell.children[0].onblur = null;
                    }
                }

                // console.log('pepe ', obj.options.data[y][x])
                // console.log('pepe value', value)
                ...
                if (obj.options.data[y][x] === value) {
                    ...
                    // console.log('3 type of ', typeof(value));
                    obj.setValue(cell, value);
                }
            } else {
                if (obj.options.columns[x].editor) {
            ...
            // NEW (by Rodrigo) REMOVE FOCUS TO AVOID ISSUES
            obj.resetSelection();
        ...
        obj.setValue = function(cell, value, force) {
            ...
            // console.log('setValue cell ', cell);
            ...
            if (typeof(cell) == 'string') {
                ...
                if (x != null && y != null) {
                    ...
                    // NEW: Updated by Rodrigo to prevent undefined values in our IU
            ...
            // NEW: Updated by Rodrigo to prevent unnecessary renders in our UI
            if (records.length) {
                // Update history
                obj.setHistory({
                    action:'setValue',
        ...
        obj.updateCell = function(x, y, value, force) {
            ...
            // NEW: Updated by Rodrigo to prevent errors on entering data quickly
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    // NEW (rodrigo)
                    var dataCopy = [...obj.options.data];
                    var rowDataCopy = [...dataCopy[y]];
                    rowDataCopy[x] = value;
                    dataCopy[y] = rowDataCopy;

                    // console.log('VALUE IN LIB ', value);
                    // console.log('1 type of ', typeof(value));

                    obj.options.data = dataCopy;
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                        ...
                        var dataCopy = [...obj.options.data];
                        var rowDataCopy = [...dataCopy[y]];
                        rowDataCopy[x] = typeof(value) === 'number' ? Number(value) : value;
                        dataCopy[y] = rowDataCopy;

                        // console.log('VALUE IN LIB ', value);
                        // console.log('2 type of ', typeof(value));

                        obj.options.data = dataCopy;
                        // obj.options.data[y][x] = value;  // OLD
        ...
        obj.updateSelectionFromCoords = function(x1, y1, x2, y2, origin) {
            ...
            obj.dispatch('onselection', obj, el, borderLeft, borderTop, borderRight, borderBottom, origin);
        ...
        obj.moveRow = function(o, d, ignoreDom) {
            ...
            // NEW: Update event function for each row
            var onmouseenterHandler = function(rowId) {
                return function() {
                    let newRowId = rowId-1;
                    // obj.rows[newRowId].style.backgroundColor = 'yellow';
                    obj.dispatch('onmouseenter', event, el, obj.rows[newRowId], newRowId);
                };
            };
            var onmouseleaveHandler = function(rowId) {
                return function() {
                    let newRowId = rowId-1;
                    // obj.rows[newRowId].style.backgroundColor = 'red';
                    obj.dispatch('onmouseleave', event, el, obj.rows[newRowId], newRowId);
                };
            };

            if (o <= d) {
                for (var rowId = 0; rowId <= d; rowId++) {
                    var belowMovement = true;
                    obj.rows[rowId].onmouseenter = onmouseenterHandler(rowId+1);
                    obj.rows[rowId].onmouseleave = onmouseleaveHandler(rowId+1);
                }
            } else {
                for (var rowId = 0; rowId <= o; rowId++) {
                    let belowMovement = false;
                    obj.rows[rowId].onmouseenter = onmouseenterHandler(rowId+1);
                    obj.rows[rowId].onmouseleave = onmouseleaveHandler(rowId+1);
                }
            }
        ...
        obj.updateTable = function() {
            ...
            if (typeof(obj.options.updateTable) == 'function') {
                ...
                for (var j = 0; j < obj.rows.length; j++) {
                    for (var i = 0; i < obj.headers.length; i++) {
                        // NEW added more params by rodrigo
                        obj.options.updateTable(el, obj.records[j][i], i, j, obj.options.data[j][i], obj.records[j][i].innerText, jexcel.getColumnNameFromId([i, j]), obj.options.data, obj);
                    }
                }
        ...
        if (jexcel.current) {
            ...
            if (! jexcel.current.edition && jexcel.current.selectedCell) {
                ...
                if (e.which == 37) {
                    ...
                    // console.log('HOLAAAA 222')
                    // Move cursor
                    if (e.shiftKey) {
                        // console.log('HOLAAAA 2.1')
                        jexcel.current.up();
                    } else if ((jexcel.current.options.allowInsertRow == true) && (jexcel.current.options.allowManualInsertRow == true)) {
                        // console.log('HOLAAAA 2.2')
                        if (jexcel.current.selectedCell[1] == jexcel.current.options.data.length - 1) {
                            // New record in case selectedCell in the last row
                            jexcel.current.insertRow();
                        }
                        // jexcel.current.down();
                    } else { // NEW: This is to allow 'Enter' key to enter edit mode
                        // console.log('jexcel.current ', jexcel.current)
                        var rowId = jexcel.current.selectedCell[1];
                        var columnId = jexcel.current.selectedCell[0];
                        if (jexcel.current.selectedCell) {
                        // if (jexcel.current.selectedCell && jexcel.current.options.columns[columnId].editor) {
                            if (jexcel.current.options.editable == true) {
                                // If is not readonly
